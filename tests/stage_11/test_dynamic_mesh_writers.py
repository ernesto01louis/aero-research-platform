...
from aero.adapters.openfoam import cylinder as cyl
...
def test_shared_transient_writers_match_static_cylinder() -> None:
    # No regression to the Stage-10 transient-cylinder GO: the shared writers render
    # byte-identically to the cylinder's original local ones for the no-motion case.
    assert fc.transient_fvschemes() == cyl._fvschemes()
    assert fc.transient_fvsolution() == cyl._fvsolution()
