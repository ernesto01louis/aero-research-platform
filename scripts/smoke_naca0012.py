#!/usr/bin/env python3
"""Stage 4 smoke script: validate pre-flight + POST 01-naca0012-baseline.

Pre-flight checks (audit D.3 + Stage 2 deviations):
    1. Prefect server is UP — citation-grade evidence bundles require it
       (without it the orchestrator falls back to `.fn` and degrades the
       LlmCall logs).
...
DEFAULT_CAMPAIGN_YAML = REPO_ROOT / "campaigns" / "01-naca0012-baseline.yaml"
DEFAULT_RESULTS_DIR = REPO_ROOT / "results" / "01-naca0012-baseline"
...
def check_templates_pushed(target_user: str = "aero", target_host: str = "192.168.2.231") -> tuple[bool, str]:
    """SSH into the target and check ~/templates/naca0012-simpleFoam/ exists."""
    import subprocess
    ...
    cmd = [
        ...
        "ls ~/templates/naca0012-simpleFoam/system/controlDict 2>/dev/null && "
        "ls ~/aero-research-platform/aero_research_platform/meshing/airfoil_cmesh.py 2>/dev/null",
    ]
    res = subprocess.run(cmd, capture_output=True, text=True, timeout=15)
    if res.returncode == 0 and "controlDict" in res.stdout and "airfoil_cmesh.py" in res.stdout:
...
def _cli() -> int:
    ...
    fails: list[str] = []
    print("=" * 64)
    print(f"Stage-4 pre-flight @ {time.strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 64)
    ...
    # The SDK already validated; we POST the raw dict because the orchestrator's
    # CampaignCreate model owns the canonical schema. Match the SDK shape one
    # last time by going through it.
