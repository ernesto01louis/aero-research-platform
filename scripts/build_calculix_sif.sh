...
#
# Stage 19 (ADR-035) — build, sign and publish the CalculiX solid-solver SIF:
# CalculiX 2.20 + the preCICE CalculiX adapter v2.20.1 + preCICE 3.4.1.
#
# This is the Stage-20 APPLICATION path, not part of the gated FSI3 claim: the
# coupling verification runs on the supported tutorial's Nutils solid (ADR-016).
#
...
#
# CalculiX and its adapter are compiled from source; allow ~15-30 min.
...
BUILD_DIR_LXC="/tmp/aero-calculix-build"

log() { echo "[build-calculix-precice-sif] $*"; }
...
ssh -o BatchMode=yes "$SSH_TARGET" "set -euo pipefail
    ...
    apptainer build --force ${SIF} calculix-precice.def
