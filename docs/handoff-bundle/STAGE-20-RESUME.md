...
passes). **C2/C3 (`23e2326`), C7 (`a2fb4ce`) and C1/C6 (`7b30bd9`: the adapter patch, launcher exports, `--restart`) are in. C5 is BUILT (2026-09-16, handoff §6.81): `calculix-precice-adr045.sif` `ca1937f7…` is the solid container of record; one include-order compile error fixed on the way (`e23e3df`). What remains is the operator's R3 option → amendment → C8. **R3 AMENDED — A17 (handoff §6.83, 2026-09-16, the operator's "Option 2 as stated in §6.82"):** the two-draw yardstick (D-C1 `…-084806` is the reference draw; RMS deviation ≤ 2× the reference's over 4001–8000 per quantity, span-mean ≤ max(Q1a 2 %, 2× reference), late-window TE RMS ≤ 2× reference; (c), (d) verbatim), the **amplitude limit** (1.3–3.3 % of full plunge, first sixth of the ramp: a pass says nothing about settled cycles) and the **episode rule** (one isolated span > 10× reference RMS, ≤ 400 windows, ≥ w5000, not in the late window, (a) passing excised ⇒ `inconclusive-episode`, ONE re-probe, a second one UNRESOLVED ⇒ NO-GO) — all pre-registered before the treatment. Restart window stays 4000 (option 3 / w400 refuted by its own calibration, §6.82). Yardstick measured: thrust RMS 7.79e-6 N, lift 4.11e-4 N, power 1.97e-6 W, late TE 1.09e-5 m. **Cost corrected: ≈ 5.9 h, not 3 h** (2.60 s/window). Suite 1092 + 3 skips. **C8 in progress. Segment 1 ran and checkpointed cleanly (w2000, w4000), but the first `--restart` DIED in 31 s (§6.85): a fresh preCICE instance zero-initialises read-data, so the fluid snapped its already-deformed interface to undeformed in one window (2.32e8 N vs the control's 0.36 N) and the solid segfaulted. The SOLID checkpoint/restart is vindicated (R5 passed, state bit-identical). Fixed by A18 (§6.86): `initialize="true"` on the restart's exchanges (`rewrite_for_restart`), inert for the control, base template untouched. Suite 1096+3. The treatment is being RE-RUN fresh (new id) behind the B0 gate; watch for its w4000 checkpoint, then `--restart … --restart-window 4000`, then `--score-r3 … --out data/vv/stage20_adr045_r3.json`.** The attempt-3 B0 + stop-rule memo is drafted (§6.87). B0 ≈ 76 h before R3. Handoff §6.75 carries **nine measured corrections** to the proposed text — the ones
