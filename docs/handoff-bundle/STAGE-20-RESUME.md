...
passes). **C2/C3 (`23e2326`), C7 (`a2fb4ce`) and C1/C6 (`b3dcf5f`: the adapter patch, launcher exports, `--restart`) are in. C5 (the container build, `VARIANT=adr045 bash scripts/build_calculix_sif.sh`) is PROPOSED, not run — it is also the patch's first compilation.** **OPEN with the operator (handoff §6.79): the R3
