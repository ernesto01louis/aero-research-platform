...
passes). **C2/C3 (`23e2326`), C7 (`a2fb4ce`) and C1/C6 (`7b30bd9`: the adapter patch, launcher exports, `--restart`) are in. C5 is BUILT (2026-09-16, handoff §6.81): `calculix-precice-adr045.sif` `ca1937f7…` is the solid container of record; one include-order compile error fixed on the way (`e23e3df`). What remains is the operator's R3 option → amendment → C8. **Option 3 (restart at w400) was chosen and then REFUTED by its own calibration (§6.82: 96 % / 153 % of p2p at 401–8000) — withdrawn uncommitted; the corrected question is option 2 with a concrete two-draw yardstick (factor 2, restart stays at 4000) or decline.** **OPEN with the operator (handoff §6.79): the R3
