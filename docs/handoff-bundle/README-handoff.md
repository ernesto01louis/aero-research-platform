# Stage map (10–21) + cross-stage guardrails — the optimizer mission
...
| 18 | **Arbitrary-Geometry Ingestion + Robust Meshing** | STL/CAD/3MF ingestion; watertightness/quality gating + repair; sandboxed meshing with fallback (FFD/SDF-space optimize + CAD emission deferred to the ledger by the committed STAGE-18 prompt). | Ingest an external geometry, mesh it robustly (quality-gated), run a CFD-verified evaluation validated against a reference. | baseline/sustained |
