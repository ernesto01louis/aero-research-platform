...
def write_block_mesh_dict(spec: FlatPlateRibletMeshSpec, out_path: Path) -> Path:
    """Write the smooth-baseline periodic-channel blockMeshDict.

    A single hex block: cyclic in x (streamwise) and y (spanwise),
    ``bottomWall`` at z=0, ``symmetryPlane`` at z=delta (channel
    centreline). z-graded toward the wall for y+<1. Same domain extents
    as the riblet structured mesh so the paired baseline shares Re_tau.

    Patches:
        xMinCyclic / xMaxCyclic   cyclic pair at x=0 / x=Lx
        frontPeriodic / backPeriodic   cyclic pair at y=0 / y=Ly
        top                       symmetryPlane at z=delta
        bottomWall                wall at z=0
    """
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    Lx = spec.plate_length
    ...
    n_z_total = spec.n_z_groove + spec.n_z_bl + spec.n_z_outer
    body = (
        _of_header("dictionary", "blockMeshDict")
        + f"""
convertToMeters 1.0;
...
"""
    ...
    out_path.write_text(body)
    _LOG.info("Wrote smooth periodic-channel blockMeshDict to %s", out_path)
    return out_path
...
def write_all(spec: FlatPlateRibletMeshSpec, case_dir: Path) -> dict[str, Path]:
    """Emit the mesh files for a periodic-channel case.

    Both variants are pure blockMesh — no snappyHexMesh, no STL:

    * Riblet case (riblet_enabled=True): structured multi-block
      blockMeshDict with the blade geometry baked into the topology
      (``write_block_mesh_dict_structured``).

    * Smooth baseline (riblet_enabled=False): single-block periodic-
      channel blockMeshDict (``write_block_mesh_dict``).

    Returns ``{"blockMeshDict": ..., "meshQualityDict": ...}`` either way.
    """
    ...
    if spec.riblet_enabled:
        ...
        paths["blockMeshDict"] = write_block_mesh_dict(
            spec, case_dir / "system" / "blockMeshDict"
        )
